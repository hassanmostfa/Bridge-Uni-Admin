...
import { useUpdateAbroadCourseMutation } from "api/studyAbroadSlice";
  ...
  useEffect(() => {
    if (courseData?.data) {
      ...
      setFormData({
        ...
        // All About Section
        allAboutTitle: course.section_title,
        numUniversities: course.number_of_universities,
        numMajors: course.number_of_majors,
        topUniversities: course.universities?.map(uni => ({
          name: uni.name,
          image: uni.image
        })) || [{ image: null, name: "" }],
        popularMajors: course.study_abroad_majors?.map(major => major.popular_major.id) || [],
        
        // Proof Section
        proofDescription: course.proof_description,
        proofImages: course.proof_images?.map(img => img.url) || [],
        requirements: course.proof_requirements?.map(req => req.name) || [""],
        
        // Visa Information
        visaDescription: course.visa_description,
        visaImage: course.visa_image,
        visaAttributes: course.visa_attributes?.map(attr => ({
          name: attr.name,
          image: attr.image
        })) || [{ name: "", image: null }],
        ...
        // Summary Section
        summary: course.summary_text,
        programRequirements: course.program_requirements?.map(req => req.name) || [""]
      });
    }
  }, [courseData]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "The study abroad program has been successfully updated",
        status: "success",
        duration: 5000,
        isClosable: true,
      });
