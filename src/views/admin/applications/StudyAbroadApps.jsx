...
const StudyAbroadApps = () => {
  const { data: studyAbroadApps, refetch, isLoading } = useGetAllRequestsQuery('studyAbroad');
  ...
  useEffect(() => {
    if (studyAbroadApps?.data?.data) {
      const formattedData = studyAbroadApps.data.data.map(app => ({
        id: app.id,
        fullName: app.full_name,
        email: app.email,
        phone: app.phone,
        courseId: app.study_abroad.id,
        courseName: app.study_abroad.title_en ?? 'Study Abroad',
      }));
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await deleteRequest({ type: 'studyAbroad', id }).unwrap();
  ...
  const handlePrintSelectedOrders = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Study Abroad Applications</title>
          <style>
            @media print {
          ...
          ${tableData
            ...
            .map(order => `
              <div class="application">
                ...
                <div class="footer">
                  <p>University Admissions Office</p>
                </div>
              </div>
            `).join('')}
        </body>
      ...
      <Card flexDirection="column" w="100%" px="0px" overflowX={{ sm: 'scroll', lg: 'hidden' }}>
        <Flex px="25px" mb="8px" justifyContent="space-between" align="center">
          <Text color={textColor} fontSize="22px" fontWeight="700" lineHeight="100%">
            Study Abroad Applications ({tableData.length})
          </Text>
          <Button
            variant="darkBrand"
            color="white"
